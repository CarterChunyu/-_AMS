...
--●明細_OP日期
SELECT *
INTO #TargetData_OpenPoint_TransDetail_DateOP
FROM dbo.OpenPoint_TransDetail ot WITH(NOLOCK)
WHERE CONVERT(varchar(7),ot.CptDate) = LEFT(@TARGET_MONTH,4)+'/'+RIGHT(@TARGET_MONTH,2)
...
SELECT REPLACE(ot.CptDate,'/','') [OP日期]
	,ot.CptDate
...
FROM #TargetData_OpenPoint_TransDetail_DateOP ot WITH(NOLOCK)
...
DROP TABLE #TargetData_OpenPoint_TransDetail_DateOP